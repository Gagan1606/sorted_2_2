...
app.post('/api/create-group', requireAuth, upload.array('photos', 50), async (req, res) => {
  try {
    ...
    for (const file of req.files) {
      ...
      if (!detectResult.success || detectResult.total_faces === 0) continue;
      
      // Match each face to users
      const detectedFaces = [];
      const faceUsersInPhoto = new Set();
      
      for (const face of detectResult.faces) {
        const match = await matchFaceToUsers(face.embedding);
        
        // FIX 1: Prevent duplicate face matches in same photo
        if (match && !faceUsersInPhoto.has(match.userId.toString())) {
          detectedFaces.push({
            user: match.userId,
          ...
          faceUsersInPhoto.add(match.userId.toString());
        }
      }
      ...
      const photo = await Photo.create({
        ...
        group: group._id,
        detectedFaces,
        capturedAt: new Date(), // TODO: Extract from EXIF
        imageHash
      });
      
      processedPhotos.push(photo._id);
    }
